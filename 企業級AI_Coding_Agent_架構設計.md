...
以 ==Claude Agent SDK== 作為核心編排器（Orchestrator），結合團隊既有的版控與 CI/CD 生態系，實現從規格定義、任務開票到 TDD 實作與驗證的**端到端自動化**。
...
拋棄肥重的外部 Connector。將 `az cli` 腳本封裝為 Agent Tool，Agent 解析完規格後直接在背景打指令完成 Azure DevOps (ADO) 開票與狀態同步。Git 操作與本地測試（`npm test`）同理。

**3. 狀態機與 Human-in-the-loop**
Agent 不會盲目執行到底。在==重要節點==（如規格寫完準備開票前）主動中斷，將產出回傳介面，等待開發者點擊「**Approve & Continue**」後才進入下一階段。
...
| 角色 | 工具 | 負責範疇 |
|------|------|---------|
| **大腦與指揮官** | [[Claude_Code_OpenSpec_Superpowers_三工具協同開發實戰指南\|OpenSpec]] | 探索現況、產出結構化規格、拆解任務為 Checklist、ADO 開票 |
...
開發者在 Chat UI 提出需求後，系統動態渲染「任務建構表單」，提供 5 種情境預設包一鍵套用：

### 情境總覽

| 情境 | 適用時機 | 執行步驟 |
|------|---------|---------|
| **Epic / Feature** | 大型新功能，需嚴謹文件與完整追蹤 | 探索 → 寫規格 → ADO 開票 → TDD → 寫程式 → 驗證 → PR → 歸檔 |
...
> [!example] 適用場景
> 從零開始規劃大型新功能，需要嚴謹文件、ADO 工單追蹤與完整的 TDD 保障。

**執行流程：**
```
[探索] → [寫規格] → [任務拆解 + ADO 同步] → [TDD] → [寫程式] → [自我驗證] → [PR] → [規格歸檔]
```

OpenSpec 處理完規格與開票後，完整交棒給 Superpowers 逐步執行。每個節點都有 Human Review 斷點。
...
| ADO 整合方式 | `az cli` 封裝為 Agent Tool | 零額外依賴、可本地測試 |
| 人工介入點 | 每個工作流的 Review 斷點 | 防止 AI 盲目執行、保留決策權 |
...
- [ ] 設計 `az cli` 封裝的 Agent Tool 介面規格
- [ ] 定義 Human-in-the-loop 的斷點觸發條件與 UI 呈現方式
- [ ] 挑選一個 Pilot 功能跑完 Epic 情境的完整流程
- [ ] 評估 5 種情境是否需要新增「Code Review（人工）」步驟
