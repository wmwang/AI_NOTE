...
### 設計原則：Preset 是建議，不是硬性約束

開發者在 Chat UI 提出需求後，系統動態渲染「任務建構表單」。5 種情境 Preset 只是==預設的起點==，使用者可以在執行前**勾選微調**任何步驟：
...
> [!tip] 為什麼不寫死流程？
> 同一個團隊裡，資深工程師可能想跳過 `spec` 直接寫碼，新人可能想在 Standard 情境下也打開 `tdd`。Preset 降低選擇負擔，微調保留彈性。**編排器和 Tool 層不需要任何改動**——它們只看最終的步驟陣列，不關心這個陣列是 preset 原樣還是使用者調整過的。
...
- [ ] 實作 Slack Interactive Message 的「可微調 Preset 表單」（含步驟勾選 + 情境切換）
